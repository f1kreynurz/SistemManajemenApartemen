            ...
            <form action="{{ route('tenant.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama" id="nama"
                        ...
                        value="{{ Auth::user()->name }}" placeholder="{{ Auth::user()->name }}" readonly>
                    @error('nama')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" id="email"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        value="{{ Auth::user()->email }}" placeholder="{{ Auth::user()->email }}" readonly>
                    @error('email')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="id_apartemen" id="id_apartemen"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                        <option value="">Pilih Apartemen</option>
                        @foreach ($apartments as $apartment)
                            <option value="{{ $apartment->id }}"
                                {{ old('id_apartemen') == $apartment->id ? 'selected' : '' }}>{{ $apartment->nama }}
                            </option>
                        @endforeach
                    </select>
                    @error('id_apartemen')
                ...
                <div class="mb-4">
                    ...
                    <input name="id_user" id="id_user" type="hidden" value="{{ Auth::user()->id }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                    <input name="name" id="id_user" type="text" value="{{ Auth::user()->name }}" readonly
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                    @error('id_user')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
