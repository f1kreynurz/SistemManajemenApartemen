        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <form action="{{ route('admin.unit.update', $unit->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="id_apartemen" id="id_apartemen" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        @foreach($apartments as $apartment)
                            <option value="{{ $apartment->id }}" {{ $unit->id_apartemen == $apartment->id ? 'selected' : '' }}>
                                {{ $apartment->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nomor_unit" id="nomor_unit" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ $unit->nomor_unit }}" placeholder="Nomor Unit" required>
                    @error('nomor_unit')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="text" name="tipe" id="tipe" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ $unit->tipe }}" placeholder="Tipe Unit" required>
                    @error('tipe')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="number" name="luas" id="luas" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ $unit->luas }}" placeholder="Luas Unit" required>
                    @error('luas')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <select name="id_penyewa" id="id_penyewa" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        @foreach($tenants as $tenant)
                            <option value="{{ $tenant->id }}" {{ $unit->id_penyewa == $tenant->id ? 'selected' : '' }}>
                                {{ $tenant->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center justify-between">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Update Unit</button>
