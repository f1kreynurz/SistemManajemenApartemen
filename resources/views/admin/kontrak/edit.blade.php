        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <form action="{{ route('admin.kontrak.update', $kontrak->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="id_unit" id="id_unit"
                        ...
                        @foreach ($units as $unit)
                            <option value="{{ $unit->id }}" {{ $kontrak->id_unit == $unit->id ? 'selected' : '' }}>
                                {{ $unit->nama }}</option>
                        @endforeach
                    </select>
                    @error('id_unit')
                ...
                <div class="mb-4">
                    ...
                    <select name="id_penyewa" id="id_penyewa"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                        <option value="">Choose Penyewa</option>
                        @foreach ($penyewas as $penyewa)
                            <option value="{{ $penyewa->id }}"
                                {{ $kontrak->id_penyewa == $penyewa->id ? 'selected' : '' }}>{{ $penyewa->nama }}</option>
                        @endforeach
                    </select>
                    @error('id_penyewa')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    <label for="tanggal_berakhir" class="block text-gray-700 text-sm font-bold mb-2">Tanggal
                        Berakhir:</label>
                    <input type="date" name="tanggal_berakhir" id="tanggal_berakhir"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        value="{{ $kontrak->tanggal_berakhir }}" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="biaya_sewa" id="biaya_sewa"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        value="{{ $kontrak->biaya_sewa }}" required>
                    @error('biaya_sewa')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
